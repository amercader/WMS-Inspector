...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="serviceT" match="WMT_MS_Capabilities/Service">
        ...
        <div class="divSection">
            ...
            <div class="divSubSection" id="divGeneralInfo">
                <table>
                    <xsl:call-template name="checkElementOutputT">
                        <xsl:with-param name="element" select="/WMT_MS_Capabilities/@version" />
                    ...
                    <xsl:call-template name="checkElementOutputT">
                        <xsl:with-param name="element" select="OnlineResource/@xlink:href" />
            ...
            <div class="divSubSection" id="divAccessConditions" style="display:none">
                <xsl:choose>
                    <xsl:when test="normalize-space(Fees) or normalize-space(AccessConstraints)">
                        <table>
                            <xsl:call-template name="checkElementOutputT">
                                <xsl:with-param name="element" select="Fees" />
                                <xsl:with-param name="elementLabel">&wmsinspector.xsl.fees;:</xsl:with-param>
                            </xsl:call-template>
                            <xsl:call-template name="checkElementOutputT">
                                <xsl:with-param name="element" select="AccessConstraints" />
    ...
    <xsl:template name="contactT" match="ContactInformation">
        <xsl:choose>
            <xsl:when test="normalize-space(.)">
                <table>

                    <xsl:call-template name="checkElementOutputT">
                        <xsl:with-param name="element" select="ContactPersonPrimary/ContactPerson" />
                        <xsl:with-param name="elementLabel">&wmsinspector.xsl.contactperson;:</xsl:with-param>
                    </xsl:call-template>

                    <xsl:call-template name="checkElementOutputT">
                        <xsl:with-param name="element" select="ContactPersonPrimary/ContactOrganization" />
                        <xsl:with-param name="elementLabel">&wmsinspector.xsl.contactorganization;:</xsl:with-param>
                    </xsl:call-template>


                    <xsl:call-template name="checkElementOutputT">
                        <xsl:with-param name="element" select="ContactPosition" />
                    ...
                    <xsl:if test="normalize-space(ContactAddress)">
                        <tr>
                            <th>&wmsinspector.xsl.contactaddress;
                                <xsl:if test="normalize-space(ContactAddress/AddressType)">
                                    (<xsl:value-of select="ContactAddress/AddressType" />)
                                </xsl:if>:
                            </th>
                            <td>
                                <xsl:if test="normalize-space(ContactAddress/Address)">
                                    <xsl:value-of select="ContactAddress/Address" />
                                </xsl:if>
                                <xsl:if test="normalize-space(ContactAddress/City) or normalize-space(ContactAddress/StateOrProvince)">
                                    <div>
                                        <xsl:if test="normalize-space(ContactAddress/City)">
                                ...
                                <xsl:if test="normalize-space(ContactAddress/PostCode) or normalize-space(ContactAddress/Country)">
                                    <div>
                                        <xsl:if test="normalize-space(ContactAddress/PostCode)">
                                            <xsl:value-of select="ContactAddress/PostCode" />
                                        </xsl:if>
                                        <xsl:if test="normalize-space(ContactAddress/Country)"> -
                    ...
                    <xsl:call-template name="checkElementOutputT">
                        <xsl:with-param name="element" select="ContactVoiceTelephone" />
                        <xsl:with-param name="elementLabel">&wmsinspector.xsl.contacttelephone;:</xsl:with-param>
                    </xsl:call-template>

                    <xsl:call-template name="checkElementOutputT">
                        <xsl:with-param name="element" select="ContactFacsimileTelephone" />
                        <xsl:with-param name="elementLabel">&wmsinspector.xsl.contactfax;:</xsl:with-param>
                    </xsl:call-template>

                    <xsl:call-template name="checkElementOutputT">
                        <xsl:with-param name="element" select="ContactElectronicMailAddress" />
    ...
    <xsl:template name="capabilityT" match="WMT_MS_Capabilities/Capability">
        ...
        <div class="divSection">
                            ...
                            <td>
                                <xsl:for-each select="DCPType/HTTP/*">
                                    <div>
                                        ...
                                        <a href="{OnlineResource/@xlink:href}">
            ...
            <div class="divSubSection" id="divAvailableExceptions" style="display: none">
                <xsl:if test="normalize-space(Exception)">
                    <table>
                        <xsl:for-each select="Exception/Format">
                            <tr>
                                <td>
                                    <xsl:value-of select="." />
                                </td>
                ...
                <div class="divSubSection" id="divUserDefinedSymbolization" style="display: none">
                    <table>
                        ...
                        <xsl:call-template name="checkElementBooleanOutputT">
                            <xsl:with-param name="element" select="UserDefinedSymbolization/@SupportSLD" />
                            <xsl:with-param name="elementLabel">&wmsinspector.xsl.supportsld;:</xsl:with-param>
                            <xsl:with-param name="outputRow" select="true()"/>
                        </xsl:call-template>

                        <xsl:call-template name="checkElementBooleanOutputT">
                            <xsl:with-param name="element" select="UserDefinedSymbolization/@UserLayer" />
                            <xsl:with-param name="elementLabel">&wmsinspector.xsl.userlayer;:</xsl:with-param>
                            <xsl:with-param name="outputRow" select="true()"/>
                        </xsl:call-template>

                        <xsl:call-template name="checkElementBooleanOutputT">
                            <xsl:with-param name="element" select="UserDefinedSymbolization/@UserStyle" />
                            <xsl:with-param name="elementLabel">&wmsinspector.xsl.userstyle;:</xsl:with-param>
                            <xsl:with-param name="outputRow" select="true()"/>
                        </xsl:call-template>

                        <xsl:call-template name="checkElementBooleanOutputT">
                            <xsl:with-param name="element" select="UserDefinedSymbolization/@RemoteWFS" />
        ...
        <tr>
            <td colspan="5">
                <div id="divLayer_{$layerId}" style="display:none; margin-left:{$layerLevel }em">
                    <div class="divLegend">
                        <xsl:choose>
                            <xsl:when test="Style/LegendURL/OnlineResource/@xlink:href">
                                <img src="{Style/LegendURL/OnlineResource/@xlink:href}" alt="&wmsinspector.xsl.legend;" title="&wmsinspector.xsl.legend;"/>
                            </xsl:when>
                            <xsl:when test="/WMT_MS_Capabilities/Capability/Request/GetLegendGraphic/DCPType/HTTP/Get/OnlineResource/@xlink:href">
                                <img src="{/WMT_MS_Capabilities/Capability/Request/GetLegendGraphic/DCPType/HTTP/Get/OnlineResource/@xlink:href}REQUEST=GetLegendGraphic&amp;SERVICE=WMS&amp;VERSION=1.1.1&amp;FORMAT={/WMT_MS_Capabilities/Capability/Request/GetLegendGraphic/Format}&amp;LAYER={Name}" alt="" />
                            </xsl:when>
                            <xsl:otherwise>
                    ...
                    <table class="tableDetails">
                        ...
                        <xsl:for-each select="BoundingBox">
                            <tr>
                                <th>&wmsinspector.xsl.extent; (<xsl:value-of select="./@SRS" />):</th>
                                <td>
